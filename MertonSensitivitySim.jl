...
function MertonSensitivitySim(nsim,mu,g, S0, sigma, lambda0, lambda, T, kappa)
    eps = randn(nsim)
    ...
    res = g(S0 * exp.((mu .- 0.5 .* sigma .^ 2 - lambda0 * kappa) .* T .+ sigma .* sqrt(T) .* eps) .* (1 .+ kappa) .^ N) .* dd_lambda
    return mean(res)
end
...
for i in eachindex(lambda_values)
    ...
    for j in eachindex(lambda0init)
        lambda0init[j] = MertonSensitivitySim(100, 0.05, g, 100, 0.2, lambda0vec[j], lambda_values[i], 1, 0.1)
    end
