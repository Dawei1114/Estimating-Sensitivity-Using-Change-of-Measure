...
# struct LogNormalBasis <: VibratoBasis
#     degree::Int
# end
...
# function(basisLog::LogNormalBasis)(x::AbstractVector, y::AbstractVector)
#     m_out = Matrix{eltype(x[1]*y[1])}(undef, length(x), basis_sizeH(basisH))
#     c = 0
#     for i in 0:basisLog.degree
#         for j in 0:basisLog.degree
#             i + j ≤ basisLog.degree || continue
#             c += 1
#             m_out[:, c] .= logpoly()
#         end
#     end
...
function simulated_price_and_sensitivity_vibrato(::AntitheticVariates, nsim::Int, nsim_estimate::Int)
    ...
    basis = HermiteBasis(5)
    model(λ) = MertonModel(100.0, 1.0, 0.05, 0.2, λ, 1e-5)
