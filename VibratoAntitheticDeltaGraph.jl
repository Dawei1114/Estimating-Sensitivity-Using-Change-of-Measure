...
using Distributions, LinearAlgebra, Random, ForwardDiff, Plots
...
function price(model::MertonModel, option::OptionContract; tol = 1e-6)
    sum_so_far = eltype(model.S0*model.T*model.μ*model.σ*model.λ*model.δ)(0.)
    n = 0
    while true
        increment = poisson_pdf(model.λ, n) * bs_price(option, model.S0, model.T, model.μ, sqrt(model.σ^2 + n * model.δ^2 / model.T)) # Mean-zero jumps are simply a volatility increase, so the price is just the Black-Scholes price with an adjusted volatility; no fancy compensation terms needed. 
...
function sim_T(model::MertonModel,Z::AbstractVector, λ₀::Union{Real, AbstractVector}, nsims::Int, S₀, T::Real)
    ...
    for i in eachindex(S)
        S[i] *= rand(LogNormal(-0.5 * log(1+model.δ^2) * Nvec[i], sqrt(log(1+model.δ^2)) * sqrt(Nvec[i]))) # price at time T - Δ with jumps. Note that an empty product is 1, so this works even if Nvec[i] == 0. However, if Ydist is lognormal, what can you say about the product of independent lognormals? 
    end
    return S, Nvec
end
...
function simulated_price_and_sensitivity_vibrato(::AntitheticVariates, nsim::Int, nsim_estimate::Int, Δ::Float64)
    option = CallOption(100.0)
    basis = LaguerreBasis(5)
    model(λ) = MertonModel(100.0, 1.0, 0.05, 0.2, λ, Δ, 0.1)
    # jumps = LogNormal(-0.5 * log(1+0.1 ^ 2), log(1+0.1 ^ 2))
    λ₀ = 0.6 # 
    m_est= Vector{Float64}(undef, 11)
    ...
    for λ in 0.0:0.1:1.0
        ...
        m_est[index], _ = price_vibrato(vrt, mod, λ₀, nsim, basis, (β̂1, β̂2))
...
function lambda_d1(::AntitheticVariates, nsim::Int, nsim_estimate::Int, Δ::Float64)
    option = CallOption(100.0)
    basis = LaguerreBasis(5)
    model(λ) = MertonModel(100.0, 1.0, 0.05, 0.2, λ, Δ, 0.1)
    # jumps = LogNormal(-0.5 * log(1+0.1 ^ 2), log(1+0.1 ^ 2))
    ...
    for λ in 0.0:0.1:1.0
        ...
        # # S_rn_disc = exp.(-mod.μ * mod.T) .* option(S_final) .* radon_nikodym(λ₀ * mod.T, mod.λ * mod.T, rn) # Just a check for reference
        # # println("- Direct price: $(mean(S_rn_disc)) ($(std(S_rn_disc) / sqrt(nsim)))")
        return ForwardDiff.derivative(x -> price_vibrato(vrt, model(x), λ₀, nsim, basis, (β̂1, β̂2))[1], λ)
    end
end

function lambda_d2(::AntitheticVariates, nsim::Int, nsim_estimate::Int, Δ::Float64)
    option = CallOption(100.0)
    basis = LaguerreBasis(5)
    model(λ) = MertonModel(100.0, 1.0, 0.05, 0.2, λ, Δ, 0.1)
    # jumps = LogNormal(-0.5 * log(1+0.1 ^ 2), log(1+0.1 ^ 2))
    ...
    for λ in 0.0:0.1:1.0
        ...
        # # S_rn_disc = exp.(-mod.μ * mod.T) .* option(S_final) .* radon_nikodym(λ₀ * mod.T, mod.λ * mod.T, rn) # Just a check for reference
        # # println("- Direct price: $(mean(S_rn_disc)) ($(std(S_rn_disc) / sqrt(nsim)))")
        return ForwardDiff.derivative(x -> ForwardDiff.derivative(y -> price_vibrato(vrt, model(y), λ₀, nsim, basis, (β̂1, β̂2))[1], x), λ)
    end
end

function MSE(Δ::Float64)
    m, _ = simulated_price_and_sensitivity_vibrato(AntitheticVariates(), 2 * 100_000, 1000_000, Δ)
    option = CallOption(100.0)
    mse = Vector{Float64}(undef, 11)
    pr(λ) = price(MertonModel(100.0, 1.0, 0.05, 0.2, λ, Δ, 0.1), option)
    for λ in 0.0:0.1:1.0
        index = Int64(λ * 10 + 1)
        mse[index] = mean((m - pr(λ)) ^ 2)
    end
    return mse  
end
...
function MSE_d1(Δ::Float64)
    m= lambda_d1(AntitheticVariates(), 2 * 100_000, 1000_000, Δ)
    option = CallOption(100.0)
    mse = Vector{Float64}(undef, 11)
    pr(λ) = price(MertonModel(100.0, 1.0, 0.05, 0.2, λ, Δ, 0.1), option)
    for λ in 0.0:0.1:1.0
        index = Int64(λ * 10 + 1)
        mse[index] = mean((m - ForwardDiff.derivative(pr, λ)) ^ 2)
    end
    return mse 
end

function MSE_d2(Δ::Float64)
    m= lambda_d1(AntitheticVariates(), 2 * 100_000, 1000_000, Δ)
    option = CallOption(100.0)
    mse = Vector{Float64}(undef, 11)
    pr(λ) = price(MertonModel(100.0, 1.0, 0.05, 0.2, λ, Δ, 0.1), option)
    for λ in 0.0:0.1:1.0
        index = Int64(λ * 10 + 1)
        mse[index] = mean((m - ForwardDiff.derivative(x -> ForwardDiff.derivative(pr, x), λ)) ^ 2)
    end
    return mse 
end
...
lambda_d1(AntitheticVariates(), 100_000, 1000_000, 1/50)

p1 = plot(λs, MSE(1/30), title = "MSE in Estimating Price", label = "Δ = 1/30", xlabel = "λ", ylabel = "MSE", lw = 2) 
for k in 1:10
    plot!(p1, λs, MSE(1/ 10 ^ k), label = "Δ = 10 ^ -$(k)", lw = 2)
end


p2 = plot(λs, MSE_d1(1/30), title = "MSE in Estimating First-Order Sensitivity", label = "Δ = 1/30", xlabel = "λ", ylabel = "MSE", lw = 2) 
for k in 1:10
    plot!(p2, λs, MSE_d1(1/ 10 ^ k), label = "Δ = 10 ^ -$(k)", lw = 2)
end

p3 = plot(λs, MSE_d2(1/30), title = "MSE in Estimating Second-Order Sensitivity", label = "Δ = 1/30", xlabel = "λ", ylabel = "MSE", lw = 2) 
for k in 1:10
    plot!(p3, λs, MSE_d2(1/ 10 ^ k), label = "Δ = 10 ^ -$(k)", lw = 2)
end


display(p3)

# simulated_price_and_sensitivity_vibrato(AntitheticVariates(), 2 * 100_000, 2 * 1000_000, 1e-6)

# p2 = plot(λs, error_d1(1/30), title = "MSE in Estimating First-Order Sensitivity", label = "Δ = 1/30", xlabel = "λ", ylabel = "MSE", lw = 2
# )
