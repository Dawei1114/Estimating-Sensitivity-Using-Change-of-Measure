...
function sim_T(model::MertonModel, λ₀::Union{Real, AbstractVector}, Ydist, nsims::Int, S₀, T::Real)
    ...
    for i in eachindex(S)
        S[i] *= prod(rand(Ydist, Nvec[i])) # price at time T - Δ with jumps. Note that an empty product is 1, so this works even if Nvec[i] == 0. However, if Ydist is lognormal, what can you say about the product of independent lognormals? 
    end
    return S, Nvec
end

function sim_T_and_T_Δ(model::MertonModel, λ₀::Real, Ydist, nsims::Int)
    S_T_Δ, _ = sim_T(model, λ₀, Ydist, nsims, model.S0, model.T - model.Δ) # Simulate prices at time T - Δ
    λs = rand(Uniform(0, 2*λ₀), nsims) # Perturbed λ values for the Vibrato method, which determine the distribution of jumps on [T-Δ, T]
    S_T, _ = sim_T(model, λs, Ydist, nsims, S_T_Δ, model.Δ) # Simulate prices at time T
    return S_T_Δ, S_T, λs
end

function estimate_vibrato(model::MertonModel, option::OptionContract, λ₀::Real, Ydist, nsims::Int, basis::VibratoBasis)
    Random.seed!(42) # for reproducibility

    S_T_Δ, S_T, λs = sim_T_and_T_Δ(model, λ₀, Ydist, nsims)
...
function price_vibrato(model::MertonModel, λ₀::Real, Ydist, nsims::Int, basis::VibratoBasis, β̂::AbstractVector)
    Random.seed!(43) # for reproducibility
    S_T_Δ, Nvec = sim_T(model, λ₀, Ydist, nsims, model.S0, model.T - model.Δ) # Simulate prices at time T - Δ
...
function simulated_price_and_sensitivity_vibrato(nsim::Int, nsim_estimate::Int)
    ...
    jumps = LogNormal(-0.5 * log(1+0.1 ^ 2), sqrt(log(1+0.1 ^ 2)))
    λ₀ = 0.5
    
    println("Simulated price and sensitivity for various λ values, using $nsim simulations:")
    for λ in 0.0:0.1:1.0
        mod = model(λ)
        β̂ = estimate_vibrato(mod, option, λ₀, jumps, nsim_estimate, basis)
        m, se = price_vibrato(mod, λ₀, jumps, nsim, basis, β̂)
        println("λ: $λ, Price: $(m) ($(se)), Sensitivity: $(ForwardDiff.derivative(x -> price_vibrato(model(x), λ₀, jumps, nsim, basis, β̂)[1], λ))")

        S_final, rn = sim_T(mod, λ₀, jumps, nsim, mod.S0, mod.T)
